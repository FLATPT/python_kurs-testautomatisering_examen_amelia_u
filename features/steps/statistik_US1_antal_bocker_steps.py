...
def step_katalog_innehaller_antal_bocker(context, antal):
    ...
    book_items = context.page.locator('[data-testid^="star-"]')
...
def step_texten_visas(context, antal):
    ...
    expected_text = f"Listan har {antal} böcker."
    text_element = context.page.locator("p").filter(has_text=expected_text)
    assert text_element.is_visible(), 'Antal böcker i Statistiken matchar inte antalet i Katalogen'
